...
/*
 * Cambiar:
 * Action por --> NombreJuego"Action"
 * Elents por --> NombreJuego"Elents"
 * + En applyTo se ha introducido codigo para saltar el turno a otro jugador.
 */
 ...
 */
public class DcAction implements GameAction<DcState, DcAction> {
     ...
     */
	public DcAction(int player, int fromRow, int fromCol, int toRow, int toCol) {
		this.toRow = toRow;
        this.toCol = toCol;
        this.player = player;
	 ...
	 */
	@Override
	public DcState applyTo(DcState state) {
        ...
        */
        ...
        board[this.fromRow][this.fromCol] = DcElements.EMPTY.getValue();
        
        // Actualizamos el estado actual para pasar al siguiente.
        DcState next = new DcState(state,board);
        
        // Si este nuevo estado no tiene acciones validas, saltamos el turno.
        if(next.validActions(next.getTurn()).isEmpty()) {
        	next = new DcState(next,board);
        }
	...
	public boolean equals(Object action) {
    	...
    	if(action instanceof DcAction) {
			DcAction Action = (DcAction) action;
			return ( 
				this.fromRow == Action.fromRow && this.toRow == Action.toRow 
				&&
				this.fromCol == Action.fromCol && this.toCol == Action.toCol
				)? true : false;
		}
